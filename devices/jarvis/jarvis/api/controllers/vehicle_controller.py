...
"""
Vehicle controller for Jarvis smart CV pipeline.

Handles vehicle tracking API endpoints using clean architecture pattern.
Wraps existing vehicle tracking business logic while maintaining API contracts.
"""
...
from ...db.vehicle_db import (
    get_all_vehicles, get_vehicle_timeline, get_vehicle_stats as db_get_vehicle_stats,
    find_match, add_vehicle, update_vehicle
)
from ...video.clip_manager import merge_clips, get_storage_stats as db_get_storage_stats, cleanup_old_clips
from ...tracking.vehicle_tracker import get_tracker, TrackingConfig

logger = logging.getLogger(__name__)


# Pydantic models (preserving existing contracts)
...
class VehicleController:
    ...
    def _setup_routes(self):
        """Setup vehicle-related routes."""
        
        # Vehicle endpoints
        @self.router.get("/", response_model=List[VehicleResponse])
        async def get_vehicles():
            ...
            try:
                ...
                logger.error(f"[VEHICLE_CONTROLLER] Error getting vehicles: {e}")
                raise HTTPException(
                    status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                    detail="Failed to retrieve vehicles"
                )

        @self.router.get("/stats", response_model=VehicleStatsResponse)
        async def get_vehicle_stats():
            """Get vehicle tracking statistics"""
            try:
                stats = db_get_vehicle_stats()
                return VehicleStatsResponse(**stats)
            except Exception as e:
                logger.error(f"[VEHICLE_CONTROLLER] Error getting vehicle stats: {e}")
                raise HTTPException(
                    status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                    detail="Failed to retrieve vehicle statistics"
                )

        @self.router.get("/{vehicle_id}", response_model=VehicleResponse)
        async def get_vehicle(vehicle_id: int = Path(..., description="Vehicle ID")):
            ...
            try:
                ...
                logger.error(f"[VEHICLE_CONTROLLER] Error getting vehicle {vehicle_id}: {e}")
                raise HTTPException(
                    status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                    detail="Failed to retrieve vehicle"
                )

        @self.router.get("/{vehicle_id}/clips")
        async def get_vehicle_clips(vehicle_id: int = Path(..., description="Vehicle ID")):
            ...
            try:
                ...
                logger.error(f"[VEHICLE_CONTROLLER] Error getting clips for vehicle {vehicle_id}: {e}")
                raise HTTPException(
                    status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                    detail="Failed to retrieve clips"
                )

        @self.router.post("/{vehicle_id}/merge")
            ...
            try:
                ...
                if not clip_paths:
                    raise HTTPException(
                        ...
                        detail="No clips found for vehicle"
                    )
                ...
                logger.error(f"[VEHICLE_CONTROLLER] Error merging clips for vehicle {vehicle_id}: {e}")
                raise HTTPException(
                    status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                    detail="Failed to merge clips"
                )

        @self.router.get("/{vehicle_id}/merged/{filename}")
        async def download_merged_video(
            ...
            try:
                ...
                logger.error(f"[VEHICLE_CONTROLLER] Error downloading merged video {filename}: {e}")
                raise HTTPException(
                    status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
        ...
        @self.router.post("/tracking/start")
        async def start_tracking():
            ...
            try:
                ...
                logger.error(f"[VEHICLE_CONTROLLER] Error starting tracking: {e}")
                raise HTTPException(
                    status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                    detail="Failed to start tracking"
                )

        @self.router.post("/tracking/stop")
        async def stop_tracking():
            ...
            try:
                ...
                logger.error(f"[VEHICLE_CONTROLLER] Error stopping tracking: {e}")
                raise HTTPException(
                    status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                    detail="Failed to stop tracking"
                )

        @self.router.get("/tracking/status")
        async def get_tracking_status():
            ...
            try:
                ...
                logger.error(f"[VEHICLE_CONTROLLER] Error getting tracking status: {e}")
                raise HTTPException(
                    status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                    detail="Failed to get tracking status"
                )

        @self.router.get("/tracking/config")
        async def get_tracking_config():
            ...
            try:
                ...
                logger.error(f"[VEHICLE_CONTROLLER] Error getting tracking config: {e}")
                raise HTTPException(
                    status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                    detail="Failed to get tracking configuration"
                )

        @self.router.post("/tracking/config")
        async def update_tracking_config(request: TrackingConfigRequest):
            ...
            try:
                ...
                logger.error(f"[VEHICLE_CONTROLLER] Error updating tracking config: {e}")
                raise HTTPException(
                    status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                    detail="Failed to update tracking configuration"
                )

        # Storage management endpoints
        @self.router.get("/storage/stats")
        async def get_storage_stats():
            """Get storage statistics"""
            try:
                stats = db_get_storage_stats()
                return stats
            except Exception as e:
                logger.error(f"[VEHICLE_CONTROLLER] Error getting storage stats: {e}")
                raise HTTPException(
                    status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                    detail="Failed to get storage statistics"
                )

        @self.router.post("/storage/cleanup")
        async def run_storage_cleanup(
            ...
            try:
                ...
                logger.error(f"[VEHICLE_CONTROLLER] Error running cleanup: {e}")
                raise HTTPException(
                    status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                    detail="Failed to run storage cleanup"
                )
